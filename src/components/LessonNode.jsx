...
import { MAX_CROWNS } from '../lib/progress'

export default function LessonNode({ unit, lessonIndex, unlocked, crowns, title, offset }) {
  const done = crowns > 0
  const inner = (
    <div className="flex flex-col items-center gap-1">
      <div className="relative">
        <div
          ...
          style={unlocked ? { backgroundColor: unit.color, borderColor: 'rgba(0,0,0,0.25)' } : {}}
        >
          {unlocked ? (done ? '⭐' : unit.icon) : '🔒'}
        </div>
        {done && (
          <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 rounded-full bg-white px-1.5 py-0.5 text-[11px] font-extrabold text-duo-yellow-dark shadow">
            {'👑'.repeat(crowns)}{crowns < MAX_CROWNS ? '' : ''}
          </div>
        )}
      </div>
  ...
  return (
    <div style={{ transform: `translateX(${offset}px)` }}>
      {unlocked ? (
        <Link to={`/lesson/${unit.id}/${lessonIndex}`} className="block transition-transform hover:scale-105">
          {inner}
        </Link>
      ) : (
